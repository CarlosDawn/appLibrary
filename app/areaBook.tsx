...
export default function BookScreen() {
  ...
  const [paginaToUp, setPaginas] = useState("");
  const [linguaUp, setLingua] = useState("");

  const [imageUp, setImage] = useState("");

  const {idLivro, titulo, autor, estado, genero, lingua, paginas, image} = useLocalSearchParams<{
    idLivro: string,
    titulo: string,
    autor: string,
    estado: string,
  ...
  function LivroScreen({ navigation }: { navigation: any }) {
    const irParaTelaUpdate = () => {
      navigation.navigate("AreaUptadeScrenn", {id: 'value'});
    };
  ...
  function AreaUptadeScrenn(){
    ...
    const idUp: number = parseInt(idLivro);
    ...
    return (
      <ScrollView>
        <View style={styles.titleContainer}>
          ...
          <TextInput keyboardType="numbers-and-punctuation" onChangeText={setPaginas} value={paginaToUp}
            placeholder='Pagínas'
